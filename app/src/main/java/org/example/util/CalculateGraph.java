...
public class CalculateGraph {
  ...
  /**
   * Computes sleep debt for each night from the first one until the final one. Passes in date and
   * sleep debt values to be graphed.
   *
  ...
  public static ResultWrapper computeValues(ArrayList<Night> nights) {
    ...
    for (int i = 0; i < orderedNights.size(); i++) {
      ...
      double sleepDebt = 0;
      for (int j = 0; j <= i; j++) {
        // For now, have the sleep debt from a night's deficit be inversely proportional to how long
        // ago that deficit was
        sleepDebt +=
            sleepDeficits.get(j)
                / Math.pow((i + 1 - j), Constants.DEFICIT_DECAY_POWER)
                * Constants.DEFICIT_MULTIPLIER;
      }
